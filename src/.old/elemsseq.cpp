...
ElemsSeq::ElemsSeq(const ElemsSeq& cM)
	:	ElementBase(cM),
		m_Families(cM.m_Families.ReAllocSizeMin1()+1),
...
void ElemsSeq::AddElement(ElementBase* a_pElement,int a_nIfCreate)
{
	...
	i = m_Families.AddAndGetIndex(pElementToAdd);
...
void ElemsSeq::RemoveElement(const int& a_nIndex)
{
	...
	m_Elements.RemoveElement( a_nIndex );
	--nSize;
	...
	if( !bFound )
	{
		m_Families.RemoveElement(nFamily);
...
void ElemsSeq::RemoveFamily(const int& a_nIndex)
{
	...
	m_Families.RemoveElement(a_nIndex);
	int nSize(m_Elements.size());
							 ...
							 const STwiss& aTwis0, STwiss*const& pTwisF)const
{
	...
	int nWhere = (nWhere1 == END_OF_LATTICE) ? (m_Elements.size()-1) : nWhere1;

	SMatrix aMatrixTrans(g_cmUnitMartix);
...
{
	int i, nSizeFam(m_Families.size()), nSizeElms(m_Elements.size());
...
void ElemsSeq::ReadFromFile1( FILE* a_File, void* a_pDevices, void* a_pFounded )
{
	...
	VectorOfType2<ElementBase*>& aDevices = *( (VectorOfType2<ElementBase*>*)a_pDevices );
	VectorOfType2<ElementBase*>& aFounded = *( (VectorOfType2<ElementBase*>*)a_pFounded );
	...
	int nKind;
	int i, j, nIndex, nSize, nSizeVect( (int)aDevices.size() ), nSizeFounded(aFounded.size());
...
const int& ElemsSeq::NumberOfDevices()const
{
...
const int& ElemsSeq::NumberOfFamilies()const
{
...
SMatrix ElemsSeq::GetMatrixTrans(	const double& a_lfS )const
{
	...
	if( !nSize )return g_cmUnitMartix;
