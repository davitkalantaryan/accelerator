...
{
	m_Thread.Run(&ThreadFuncStat,this);
}
...
void* OptimAlg::CMatcherThread::ThreadFunc()
{
	...
	int64_tt iTmp, llnStep;
	int i2;
	const _type_ind_ nMagnets( m_pOwner->m_pOwner->m_Inputs.size() );
	SInputParams aInput;
	...
	memset( m_pllnStep, -1, sizeof(int64_tt)*nMagnets );


	for( int64_tt lli(m_lli1); lli < m_lli2 && *m_pWork; ++lli )
	{/// For1
...
void* OptimAlg::FinalMatcher()
{
	...
	for( int i(0); i<m_ThreadItems.size(); ++i )
	{
		...
		(pThrItem->m_Thread).WaitForThread(-1);
...
void OptimAlg::StartMatch(int nThreadsNum)
{
	...
	const _type_ind_ nMagnets = (m_pOwner->m_Inputs).size();
	if(!nMagnets)
	{
		TellOwnerOnFinish();
		return ;
	}
	m_pllnSize = (int64_tt*)realloc( m_pllnSize,sizeof(int64_tt)*nMagnets );

	OptimAlg::CMatcherThread* pThrItem;

	int i,nSize( m_ThreadItems.size() );


	int64_tt llnTotalIters, llnSize(1);
	...
	int64_tt llni1(0), llndi(llnTotalIters/nThreadsNum + (llnTotalIters%nThreadsNum?1:0)), llni2(llndi);
	...
	for( i = 0; i < nThreadsNum; ++i )
	{
		...
		llni1 += llndi;
		llni2 += llndi;
		pThrItem->m_pllnStep = (int64_tt*)malloc( sizeof(int64_tt)*nMagnets );
		m_ThreadItems.push_back(pThrItem);

		(pThrItem->m_Thread).Run( &OptimAlg::CMatcherThread::ThreadFuncStat, pThrItem );
	}

	m_MatcherTHread.Run(&OptimAlg::FinalMatcherStat, this);
	m_MatcherTHread.Detach();
...
void Matcher::SetTwiss0(const STwiss& Twiss0)
{
	...
	m_Params.m_Twiss0.ObtainGamaX();
	m_Params.m_Twiss0.ObtainGamaY();
}
...
{
	a_pMatcherFlds->m_Twiss0.ObtainGamaX();
	a_pMatcherFlds->m_Twiss0.ObtainGamaY();
